...
const showWeather = async ()=>{
    try {
        let data = await (0, _baseJs.getWeather)(cityName);
        console.log(data);
        data.results.map((weatherDataMap)=>{
            console.log(`weatherDataMap = `);
            console.log(weatherDataMap);
            (0, _weatherJs.renderWeather)(weatherDataMap, weatherData);
        });
    } catch (error) {
        console.error(error);
    }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["jy52z","1Z4Rq"], "1Z4Rq", "parcelRequired5d3")
